...
export default function Chat() {
  ...
  if (isArtist()) {
    if (loading) {
      return (
        <div className="flex items-center justify-center h-64" data-testid="chat-loading">
    ...
    return (
      <div className="flex flex-col h-[calc(100vh-180px)] lg:h-[calc(100vh-140px)]" data-testid="chat-room">
        {/* Header */}
        <div className="flex items-center gap-3 pb-4 border-b border-67-gray" data-testid="chat-header">
          <div
        ...
        {/* Messages */}
        <div className="flex-1 overflow-y-auto py-4 space-y-3 chat-scroll" data-testid="chat-messages">
          {messages.length === 0 ? (
            <div className="text-center text-gray-500 py-8" data-testid="chat-empty">
              <p>No messages yet</p>
            ...
            messages.map((message, index) => {
              ...
              return (
                <React.Fragment key={message.id}>
                  ...
                  <div className={`flex ${own ? 'justify-end' : 'justify-start'}`} data-testid="chat-message">
                    <div className={`max-w-[80%] ${own ? 'order-2' : ''}`}>
                      {!own && (
                        <p className="text-xs text-gray-500 mb-1 ml-1" data-testid="message-sender">
                          {message.sender?.display_name}
                        </p>
                      )}
                      <div className={`px-4 py-2.5 rounded-2xl ${
                        own
                          ? 'bg-67-gold text-white rounded-br-md'
                      ...
                      <p className={`text-[10px] text-gray-500 mt-1 ${own ? 'text-right mr-1' : 'ml-1'}`} data-testid="message-time">
        ...
        <div className="pt-4 border-t border-67-gray">
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-4 py-3 bg-67-gold text-white rounded-xl hover:bg-67-amber transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
  ...
  return (
    <div className="flex h-[calc(100vh-180px)] lg:h-[calc(100vh-140px)] gap-4">
      ...
      <div className={`${selectedConversation ? 'hidden lg:flex' : 'flex'} flex-col w-full lg:w-80 flex-shrink-0`}>
        ...
        {loading ? (
          <div className="flex items-center justify-center h-32" data-testid="chat-loading">
          ...
          <div className="space-y-2 overflow-y-auto flex-1" data-testid="chat-list">
            {conversations.map(conv => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-white truncate" data-testid="chat-artist-name">{conv.artist?.name}</p>
                  {conv.lastMessage && (
                    <p className="text-xs text-gray-500 truncate" data-testid="chat-preview">
      ...
      {/* Chat Area */}
      {selectedConversation ? (
        <div className="flex-1 flex flex-col bg-67-dark rounded-2xl border border-67-gray overflow-hidden" data-testid="chat-room">
          {/* Chat Header */}
          <div className="flex items-center gap-3 p-4 border-b border-67-gray" data-testid="chat-header">
            <button
            ...
            <div>
              <h3 className="font-bold text-white" data-testid="chat-artist-name">{selectedConversation.artist?.name}</h3>
              <p className="text-xs text-gray-500">Artist</p>
            </div>
          </div>

          {/* Messages */}
          <div className="flex-1 overflow-y-auto p-4 space-y-3 chat-scroll" data-testid="chat-messages">
            {messages.map((message, index) => {
              const own = isOwnMessage(message)
              const showDate = index === 0 ||
                new Date(message.created_at).toDateString() !== new Date(messages[index - 1].created_at).toDateString()
          ...
          <div className="p-4 border-t border-67-gray">
            <div className="flex gap-2">
              ...
              <button
                onClick={handleSendMessage}
                disabled={!newMessage.trim() || sending}
                className="px-4 py-3 bg-67-gold text-white rounded-xl hover:bg-67-amber transition-colors disabled:opacity-50"
                data-testid="send-message"
              >
                <Send className="w-5 h-5" />
              </button>
        ...
        <div className="hidden lg:flex flex-1 items-center justify-center bg-67-dark rounded-2xl border border-67-gray" data-testid="chat-empty">
